...
extern "C" void app_main()
{
    ...
    app_driver_handle_t light_handle2 = app_driver_light_init();
    ...
    // node handle can be used to add/modify other endpoints. This is Endpoint 0.
    node_t *node = node::create(&node_config, app_attribute_update_cb, app_identification_cb);
    ABORT_APP_ON_FAILURE(node != nullptr, ESP_LOGE(TAG, "Failed to create Matter node"));

    MEMORY_PROFILER_DUMP_HEAP_STAT("node created");

    /* Create on off light endpoint01 */
    ...
    attribute::set_deferred_persistence(onoff_attribute);

    door_lock::config_t door_lock_config;
    cluster::door_lock::feature::user::config_t user_config;

    endpoint_t *endpoint2 = door_lock::create(node, &door_lock_config, ENDPOINT_FLAG_NONE, NULL);
    ABORT_APP_ON_FAILURE(endpoint2 != nullptr, ESP_LOGE(TAG, "Failed to create door lock endpoint"));


    door_endpoint_id2 = endpoint::get_id(endpoint2);
    ESP_LOGI(TAG, "Light created with endpoint_id %d", door_endpoint_id2);

    /* Mark deferred persistence for some attributes that might be changed rapidly */
    onoff_attribute = attribute::get(door_endpoint_id2, OnOff::Id, OnOff::Attributes::OnOff::Id);

        door_lock_init();

    MEMORY_PROFILER_DUMP_HEAP_STAT("light created");
